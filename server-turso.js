// ===== SERVER.JS COM TURSO - Backend API para Painel de Consultorias TR =====
...
const { dbGet, dbAll, dbRun, initializeDatabase } = require('./db-turso');
...
initializeDatabase().catch(err => {
    console.error('Erro fatal ao inicializar database:', err);
    process.exit(1);
});
...
app.listen(PORT, () => {
    ...
    console.log(`🔑 JWT Secret: ${JWT_SECRET !== 'tr-sped-secret-key-change-in-production' ? '✅ Configurado' : '⚠️ Use .env'}`);
    console.log(`🗄️ Database: ${process.env.TURSO_DATABASE_URL ? '☁️ Turso' : '💾 SQLite Local'}`);
});
